%%%%% OdpowiedŸ skokowa modelu rozmytego wzglêdem wartoœci h2
...
[A, B, X0, U0, range] = linAB_TS_X(data, n, rng);       % A, B, X0, U0 - parametry n zlinearyzowanych uk³adów
...
solution1 = ode45(@(t, x)plant_TS_X(t, x, control_delay(t, controlhandle(:,1)), X0, U0, A, B, n, U0_, range), T, X0_, options);
[~, t_U1, U1] = control_delay([], [], 1);         % Przebieg sterowania
control_delay([], [], 0);                       % Reset funkcji realizuj¹cej opóŸnienie
solution2 = ode45(@(t, x)plant_TS_X(t, x, control_delay(t, controlhandle(:,2)), X0, U0, A, B, n, U0_, range), T, X0_, options);
[~, t_U2, U2] = control_delay([], [], 1);         % Przebieg sterowania
control_delay([], [], 0);                       % Reset funkcji realizuj¹cej opóŸnienie
solution3 = ode45(@(t, x)plant_TS_X(t, x, control_delay(t, controlhandle(:,3)), X0, U0, A, B, n, U0_, range), T, X0_, options);
[~, t_U3, U3] = control_delay([], [], 1);         % Przebieg sterowania
control_delay([], [], 0);                       % Reset funkcji realizuj¹cej opóŸnienie
solution4 = ode45(@(t, x)plant_TS_X(t, x, control_delay(t, controlhandle(:,4)), X0, U0, A, B, n, U0_, range), T, X0_, options);
