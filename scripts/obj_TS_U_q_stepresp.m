%%%%% OdpowiedŸ skokowa modelu rozmytego wzglêdem wartoœci h2 rozmycie
%%%%% geometryczne
...
% skrajne modele s¹ wyznaczone wokó³ punktów h_pracy_min/max = h_20 -/+ rng * h20
n = 5;                                                  % liczba punktów pracy
rng = 0.5;                                              % zakres rozmywania
q = 1.3;                                                % krok rozmywania
[A, B, X0, U0, range, activ_U] = linAB_TS_U_q(data, n, rng, q);       % A, B, X0, U0 - parametry n zlinearyzowanych uk³adów
[~,~,X0_, U0_] = linAB(data.h20, data);                 % wektor sterowania i stanu w zadanym punkcie pracy

%% Parametry symulacji
step = [1; 0];                                  % Skok jedynie wartoœci steruj¹cej
step_val = 40*[-1,-0.5, 0.5, 1];                % Przyrosty wartoœci steruj¹cej F = F1 + FD + step_val 
...
solution1 = ode45(@(t, x)plant_TS_U_q(t, x, control_delay(t, controlhandle(:,1)), X0, U0, A, B, n, U0_, activ_U), T, X0_, options);
[~, t_U1, U1] = control_delay([], [], 1);         % Przebieg sterowania
control_delay([], [], 0);                       % Reset funkcji realizuj¹cej opóŸnienie
solution2 = ode45(@(t, x)plant_TS_U_q(t, x, control_delay(t, controlhandle(:,2)), X0, U0, A, B, n, U0_, activ_U), T, X0_, options);
[~, t_U2, U2] = control_delay([], [], 1);         % Przebieg sterowania
control_delay([], [], 0);                       % Reset funkcji realizuj¹cej opóŸnienie
solution3 = ode45(@(t, x)plant_TS_U_q(t, x, control_delay(t, controlhandle(:,3)), X0, U0, A, B, n, U0_, activ_U), T, X0_, options);
[~, t_U3, U3] = control_delay([], [], 1);         % Przebieg sterowania
control_delay([], [], 0);                       % Reset funkcji realizuj¹cej opóŸnienie
solution4 = ode45(@(t, x)plant_TS_U_q(t, x, control_delay(t, controlhandle(:,4)), X0, U0, A, B, n, U0_, activ_U), T, X0_, options);
...
%% Wykresy
figure('NumberTitle', 'off', 'Name', 'OdpowiedŸ modelu rozmytego wzglêdem h2 na skok');
subplot(2,2,1);
...
title('Przebieg wartoœci sterowania i dop³ywu', 'interpreter', 'latex');
legend('skok 1','skok 2','skok 3','skok 4', 'location', 'east');
grid on;
...
title('Przebieg wartoœci zak³ócenia', 'interpreter', 'latex');
grid on;
